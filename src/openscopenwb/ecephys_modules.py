...
def ecephys_align_timestamps(module_params):
    ...
    if alt_probe_directory != []:
        try:
            np.load(glob(join(alt_probe_directory,
                    "spike_times.npy"))[0])
            spike_directory = glob(join(alt_probe_directory,
                    "spike_times.npy"))[0]
            file_found = True
            file_in_parent_folder = True

        except FileNotFoundError:
            print(' Spikes not found for ' + join(module_params['base_directory'],
    ...
    if(queue_directory != []) and file_in_parent_folder == False:
        try:
            np.load(join(queue_directory, 'spike_times.npy'))
            alt_spike_directory = glob(join(queue_directory, "spike_times.npy"))[0]
            file_found = True
    ...
    if(file_in_probe_folder):
        timestamp_files.append({
            ...
            'output_path': join(output_directory, probe_idx, 'spike_times_master_clock.npy')
        })
...
def ecephys_write_nwb(module_params):
    ...
    if search_child:
        ...
        if child_neuropix != []:
            try:
                channel_info = pd.read_csv(glob(join(module_params['base_directory'],
                                '*', "*" + probe_idx,
                                'continuous',
                                'Neuropix*',
                                "ccf_regions.csv"))[0])
                ...
                channel_in_child = False
    if channel_in_child == False and channel_in_parent == False:
        search_queue = True
    
    if search_queue:
        queue_neuropix = glob(os.path.join(
        base_directory, 'EUR_QUEUE*', 'continuous', 'Neuropix*', "ccf_regions.csv"))
        if queue_neuropix != []:
            try:
    ...
    if channel_in_parent:
        ...
        for idx, channel_row in channel_info.iterrows():
            structure_acronym = channel_row['structure_acronym']
            channel_dict = {
                'id': idx * 1000,
                'valid_data': True,
                'probe_id': 1,
        ...
        for idx, unit_row in unit_info.iterrows():
            if quality_info.loc[unit_row.cluster_id].group != 'noise': 
                spike_count = np.sum(spike_clusters ==
                                     unit_row['cluster_id'])
                unit_dict = {
                    'id': module_params['last_unit_id'],
        ...
        probe_dict = {
            'id': module_params['id'],
            'name': probe_idx,
            'spike_times_path': join(output_directory, probe_idx,
                                     'spike_times_master_clock.npy'),
            'spike_clusters_file': join(probe_directory, 'spike_clusters.npy'),
            'spike_amplitudes_path': join(probe_directory, 'amplitudes.npy'),
            'mean_waveforms_path': join(probe_directory, 'mean_waveforms.npy'),
