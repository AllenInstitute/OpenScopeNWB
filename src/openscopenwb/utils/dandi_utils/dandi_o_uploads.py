...
def automatic_dandi_upload(
    ...
    ----------
    dandiset_id : str
        Six-digit string identifier for the DANDISet the NWBFiles
        will be uploaded to.
    ...
    dandiset_folder_path : folder path, optional
        ...
        Used in cases where you do not have write permissions for the
        parent of the 'nwb_folder_path' directory.
        Default behavior downloads the DANDISet to a folder adjacent
        to the 'nwb_folder_path'.
    ...
    dandi_download(urls=r'https://dandiarchive.org/dandiset/000336/draft/',
    ...
    organized_nwbfiles = glob(
        join(directory_path, dandiset_id,  'sub-' + subject_id,  "*.nwb"))
    ...
    for organized_nwbfile in organized_nwbfiles:
        ...
        if "ses" not in file_path.stem:
            ...
            if raw == "True":
                dandi_stem_split.insert(
                    1, f"ses-{session_id}-acq-{experiment_id}raw")
            else:
                dandi_stem_split.insert(
                    1, f"ses-{session_id}-acq-{experiment_id}")
    ...
    if args.final == "True":
        fb.update_session_dandi("OpenScopeDendriteCoupling",
                                session_id, "sub_" + args.subject_id +
                                '/' + session_id)
    fb.update_session_status("OpenScopeDendriteCoupling",
                             args.session_id, "Uploaded")
...
if __name__ == '__main__':
    ...
    raw = args.raw
    final = args.final
    automatic_dandi_upload(nwb_folder_path=nwb_folder_path,
