...
def test_dandi_download(dandi_prep):
    ...
    dandi_download(urls=r'https://dandiarchive.org/dandiset/' +
                   dandi_prep['dandiset_id']
...
def test_dandi_upload(dandi_prep):
    ...
    os.environ['DANDI_API_KEY'] = cuf.get_creds()
    cmd = "./scripts/dandi_ephys_uploads.py --nwb_folder_path " + \
          dandi_prep['file_path'] + \
          " --dandiset_id " + dandi_prep['dandiset_id'] + " --sess_id " + \
          dandi_prep['session_id'] + " --project_id " + \
