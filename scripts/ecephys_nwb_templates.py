...
from pynwb.image import ImageSeries, IndexSeries, Images, GrayscaleImage, RGBImage
from allensdk.brain_observatory.behavior.data_objects.stimuli.presentations \
    import \
    Presentations
if __name__ == "__main__":
    directory = r'/allen/programs/mindscope/workgroups/openscope/tiff_stim_files/illusion'
                ...
                # Loop through each file in the folder
                    ...
                    for filename in os.listdir(os.path.join(directory, folder)):
                        ...
                        if filename.endswith(".tif") or filename.endswith(".tiff"):
                            ...
                            image = np.array(Image.open(os.path.join(directory, folder, filename)))
                            ...
                            for i in range(x):
                                for j in range(y):
                                    image_list.append((len(image_list), i, j, image[i, j])) 
                        data = Image.open(os.path.join(directory, folder, filename))
                        data = np.array(data)
                        data_list = []
                        times = []
